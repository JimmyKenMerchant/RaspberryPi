...
int32 _user_start() {
	...
	result = _make_cl_binning( width_pixel, height_pixel, flag_multi );
print32_debug( result, 0, 0 );
	//_unmake_cl_binning();
	result = _make_cl_rendering( FB32_FRAMEBUFFER->addr, width_pixel, height_pixel, flag_multi );
print32_debug( result, 0, 300 );
	//_unmake_cl_rendering();
	//_clear_cl_rendering( uint32 clear_color, uint32 clear_z, uchar8 clear_alpha, uchar8 clear_stencil );
