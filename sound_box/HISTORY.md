...
* * Raspberry Pi 3 B V.1.2 (BCM2837), `make type=3b`
	...
	* PWM Output GPIO 40 (PWM0) and GPIO 41 (PWM1) on `sound=jack`, Including Tuning of E4 with Triangle Wave (Command 5) > OK
	* Balanced PWM Output GPIO 40 (PWM0) and GPIO 41 (PWM1) on `sound=jackb`, Including Tuning of E4 with Triangle Wave (Command 5) and Confirmation of Counteraction by USB Mono Microphone Input with Recording by Audacity > OK
	...
	* Playing Signal (GPIO 16) at `sound=jack` and `sound=i2s`: Connected with 3mm Red LED and 10K ohms resister to limit current. > OK
	* Synchronization Clock OUT (GPIO 17) at `sound=jack` and `sound=i2s`: Connected with 3mm Red LED and 10K ohms resister to limit current. > OK
	* MIDI IN at `sound=jack`
