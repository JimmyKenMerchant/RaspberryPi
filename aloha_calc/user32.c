...
typedef enum _command_list {
	...
	div,
	rem,
	addu, // Integer unsigned addition, "addu @D @S1 @S2": D = S1 + S2. 0x0 through 0xFFFFFFFF.
	subu,
	mulu,
	divu,
	remu,
	...
	_if, // IF statement, "if @S1 <Comparison Symbol> @S2": Compare S1 and S2 as singed integer. If the comparison is false, skip the next line.
	ifu, // IF statement, "ifu @S1 <Comparison Symbol> @S2": Compare S1 and S2 as unsinged integer. If the comparison is false, skip the next line.
	tst, // Logical comparison, "tst @S1 @S2": Reflects NZCV flags.
	addb, // Binary-coded decimal, "addb @D @S1 @S2": D = S1 + S2. -9,999,999,999,999,999 through 9,999,999,999,999,999.
	subb,
	mulb,
	divb,
	remb,
	cmpb, // Compare two values of binary-coded decimal, "cmpb @S1 @S2": Reflects NZCV flags.
	ifb, // IF statement, "ifb @S1 <Comparison Symbol> @S2": Compare S1 and S2 as binary-coded decimal. If the comparison is false, skip the next line.
	addf, // Floating point addition, "addf @D @S1 @S2": D = S1 + S2.
	subf, // Floating point subtruction, "subf @D @S1 @S2": D = S1 - S2.
	mulf,
	divf,
	...
	iff, // IF statement, "iff @S1 <Comparison Symbol> @S2": Compare S1 and S2 as floating point. If the comparison is false, skip the next line.
	input, // Input to the line, "input @D".
	cmps, // String Compare
	ifs, // IF statement, "ifs @S1 <Comparison Symbol> @S2": Compare S1 and S2 as string. If the comparison is false, skip the next line.
	mov, // Copy, "mov @D @S1".
	apd, // Append, "apd @D @S1".
...
/**
 * IF statements (if, ifu, ifb, iff, ifs) are special:
				...
				switch ( pipe_type ) {
					case search_command:
						...
						if ( str32_strmatch( temp_str, 1, "*\0", 1 ) ) {
							...
							pipe_type = enumurate_sources;
							src_index = 1; // 0 is Start Point
						} else if ( str32_strmatch( temp_str, 4, "add \0", 4 ) ) {
							command_type = add;
							length_arg = 3;
							pipe_type = enumurate_sources;
							src_index = 1; // 0 is Direction
						} else if ( str32_strmatch( temp_str, 4, "sub \0", 4 ) ) {
							command_type = sub;
							length_arg = 3;
							pipe_type = enumurate_sources;
							src_index = 1; // 0 is Direction
						} else if ( str32_strmatch( temp_str, 4, "mul \0", 4 ) ) {
							command_type = mul;
							length_arg = 3;
							pipe_type = enumurate_sources;
							src_index = 1; // 0 is Direction
						} else if ( str32_strmatch( temp_str, 4, "div \0", 4 ) ) {
							command_type = div;
							length_arg = 3;
							pipe_type = enumurate_sources;
							src_index = 1; // 0 is Direction
						} else if ( str32_strmatch( temp_str, 4, "rem \0", 4 ) ) {
							command_type = rem;
							length_arg = 3;
							pipe_type = enumurate_sources;
							src_index = 1; // 0 is Direction
						} else if ( str32_strmatch( temp_str, 5, "addu \0", 5 ) ) {
							command_type = addu;
							length_arg = 3;
							pipe_type = enumurate_sources;
							src_index = 1; // 0 is Direction
						} else if ( str32_strmatch( temp_str, 5, "subu \0", 5 ) ) {
							command_type = subu;
							length_arg = 3;
							pipe_type = enumurate_sources;
							src_index = 1; // 0 is Direction
						} else if ( str32_strmatch( temp_str, 5, "mulu \0", 5 ) ) {
							command_type = mulu;
							length_arg = 3;
							pipe_type = enumurate_sources;
							src_index = 1; // 0 is Direction
						} else if ( str32_strmatch( temp_str, 5, "divu \0", 5 ) ) {
							command_type = divu;
							length_arg = 3;
							pipe_type = enumurate_sources;
							src_index = 1; // 0 is Direction
						} else if ( str32_strmatch( temp_str, 5, "remu \0", 5 ) ) {
							command_type = remu;
							length_arg = 3;
							pipe_type = enumurate_sources;
							src_index = 1; // 0 is Direction
							...
							pipe_type = enumurate_sources;
							src_index = 0; // No Direction
						} else if ( str32_strmatch( temp_str, 5, "addb \0", 5 ) ) {
							command_type = addb;
							length_arg = 3;
							pipe_type = enumurate_sources;
							src_index = 1; // 0 is Direction
						} else if ( str32_strmatch( temp_str, 5, "subb \0", 5 ) ) {
							command_type = subb;
							length_arg = 3;
							pipe_type = enumurate_sources;
							src_index = 1; // 0 is Direction
						} else if ( str32_strmatch( temp_str, 5, "mulb \0", 5 ) ) {
							command_type = mulb;
							length_arg = 3;
							pipe_type = enumurate_sources;
							src_index = 1; // 0 is Direction
						} else if ( str32_strmatch( temp_str, 5, "divb \0", 5 ) ) {
							command_type = divb;
							length_arg = 3;
							pipe_type = enumurate_sources;
							src_index = 1; // 0 is Direction
						} else if ( str32_strmatch( temp_str, 5, "remb \0", 5 ) ) {
							command_type = remb;
							length_arg = 3;
							pipe_type = enumurate_sources;
							src_index = 1; // 0 is Direction
							...
							pipe_type = enumurate_sources;
							src_index = 0; // No Direction
						} else if ( str32_strmatch( temp_str, 5, "addf \0", 5 ) ) {
							command_type = addf;
							length_arg = 3;
							pipe_type = enumurate_sources;
							src_index = 1; // 0 is Direction
						} else if ( str32_strmatch( temp_str, 5, "subf \0", 5 ) ) {
							command_type = subf;
							length_arg = 3;
							pipe_type = enumurate_sources;
							src_index = 1; // 0 is Direction
						} else if ( str32_strmatch( temp_str, 5, "mulf \0", 5 ) ) {
							command_type = mulf;
							length_arg = 3;
							pipe_type = enumurate_sources;
							src_index = 1; // 0 is Direction
						} else if ( str32_strmatch( temp_str, 5, "divf \0", 5 ) ) {
							command_type = divf;
							length_arg = 3;
							pipe_type = enumurate_sources;
							src_index = 1; // 0 is Direction
							...
							length_arg = 2;
							pipe_type = execute_command;
						} else if ( str32_strmatch( temp_str, 4, "mov \0", 4 ) ) {
							command_type = mov;
							length_arg = 2;
							pipe_type = execute_command;
						} else if ( str32_strmatch( temp_str, 4, "apd \0", 4 ) ) {
							command_type = apd;
							length_arg = 2;
							pipe_type = execute_command;
...
/*print32_debug( var_temp2.u32, 300, 300  );*/
					...
					case enumurate_sources:
						...
						/*  Pass Spaces and Label*/
						temp_str = pass_space_label( UART32_UARTINT_HEAP );
						if ( command_type >= addf ) { // Type of Single Precision Float
						...
						} else if ( command_type >= addb ){ // Type of Binary-coded Decimal
